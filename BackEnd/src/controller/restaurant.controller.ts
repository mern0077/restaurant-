...
export const getRestaurantOrder = async (req: any, res: any) => {
    try {
        const restaurant = await Restaurant.findOne({ user: req.id });
        if (!restaurant) {
            return res.status(404).json({
...
export const searchRestaurant = async (req: any, res: any) => {
    try {
...
export const getSingleRestaurant = async (req:any, res:any) => {
    try {
